...
import {
  ...
  Check, Trash2, Info, AlertCircle, CheckCircle,
  Loader2, RefreshCw, FolderKanban
} from 'lucide-react';
...
function buildNotifications(contacts, leads, tickets, proposals) {
  ...
  tickets.filter(t => t.status === 'open').slice(0, 2).forEach(t => {
    items.push({
      id: `ticket-${t._id}`,
      type: t.priority === 'critical' || t.priority === 'high' ? 'warning' : 'info',
      title: t.priority === 'critical' ? '🚨 Critical ticket open' : 'Open ticket',
      desc: t.title,
      time: t.createdAt,
      read: false,
      link: '/tickets',
  ...
  // Expiring proposals (valid until within 3 days)
  ...
  proposalList
    ...
    const projectList = Array.isArray(proposals) ? proposals : proposals?.data || [];
    projectList.filter(p => p.status === 'delayed').slice(0, 2).forEach(p => {
      items.push({
        id: `project-delayed-${p._id}`,
        type: 'warning',
...
function NotificationsDropdown() {
  ...
  const fetchNotifications = useCallback(async () => {
    setLoading(true);
    try {
      const [contacts, leads, tickets, proposals] = await Promise.all([
        api.get('/contacts'),
        api.get('/leads'),
        api.get('/tickets'),
        api.get('/proposals'),
      ...
      setNotifications(buildNotifications(contacts.data, leads.data, tickets.data, proposals.data, projects.data));
    } catch (err) {
      // silently fail
    } finally {
      setLoading(false);
      ...
      {open && (
        <div className="absolute right-0 top-11 w-80 bg-white rounded shadow-lg border border-gray-200 z-50">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
            ...
            <div className="flex items-center gap-2">
              <button onClick={fetchNotifications} className="text-gray-400 hover:text-gray-600 transition-colors" title="Refresh">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <button onClick={(e) => dismiss(n.id, e)} className="text-gray-300 hover:text-red-400 transition-colors flex-shrink-0 mt-0.5">
...
function UserDropdown() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get('/proposals'),
      api.get('/projects'),
    ]).then(([me, contacts, leads, tickets, proposals, projects]) => {
      setProfile(me.data);
      const proposalList = Array.isArray(proposals.data) ? proposals.data : proposals.data?.data || [];
      setStats({
        contacts:  contacts.data.length,
        leads:     leads.data.length,
        tickets:   tickets.data.filter(t => t.status === 'open').length,
      ...
      {open && (
        <div className="absolute right-0 top-11 w-64 bg-white rounded shadow-lg border border-gray-200 z-50">
          <div className="px-4 py-3 border-b border-gray-100">
            ...
            {/* Live stats — now 4 items */}
            {stats ? (
              <div className="grid grid-cols-4 gap-1 mt-3 pt-3 border-t border-gray-100">
                {[
                  { label: 'Contacts',  value: stats.contacts,  link: '/contacts'  },
                  { label: 'Leads',     value: stats.leads,     link: '/leads'     },
                  { label: 'Tickets',   value: stats.tickets,   link: '/tickets'   },
                  { label: 'Proposals', value: stats.proposals, link: '/proposals' },
      ...
      <nav className="flex-1 py-3 overflow-y-auto">
        {navGroups.map((group, gi) => (
          <div key={gi} className="mb-2">
            {group.label && (
              <p className="px-4 py-2 text-xs font-semibold uppercase tracking-widest" style={{ color: '#8b92a5' }}>
                {group.label}
              </p>
            )}
            {group.items.map(({ path, label, icon: Icon }) => {
              ...
              return (
                <Link
                  ...
                  onMouseEnter={e => { if (!active) { e.currentTarget.style.backgroundColor = '#2d3348'; e.currentTarget.style.color = '#fff'; } }}
                  onMouseLeave={e => { if (!active) { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = '#8b92a5'; } }}
                >
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute left-0 top-0 bottom-0 w-56 flex flex-col">
            ...
            <button onClick={() => setSidebarOpen(false)} className="absolute top-4 right-3 text-white opacity-60 hover:opacity-100">
              <X className="w-5 h-5" />
            </button>
          </aside>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-gray-200 flex items-center gap-3 px-4 h-14 flex-shrink-0">
          <button className="lg:hidden text-gray-500 hover:text-gray-700 mr-1" onClick={() => setSidebarOpen(true)}>
